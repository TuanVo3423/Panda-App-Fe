...
export const NotificationScreen = (props: any) => {
  ...
  useEffect(() => {
    const allNotiChecked = ['noti1', 'noti2', 'noti3'].every(id => checkboxStates[id]);
  ...
  return <SafeAreaView className='bg-transparent'>
    <View className='bg-white h-full'>
      <View className='flex-row bg-white flexitem items-center space-x-28 px-5 justify-center h-[70px]'>
        <Feather name='chevron-left' size={28} className='' onPress={() => navigation.goBack()}/>
        <Text className='text-lg font-semibold'>Notifications</Text>
        <Feather name='check-square' size={25} onPress={() => setShowCheckboxes(!showCheckboxes)} />
      </View>
